...
def mysql_database_names(service: str, password: str) -> List[str]:
    ...
    names = [line.strip() for line in result.stdout.decode(errors="ignore").splitlines() if line.strip()]
...
def create_backup_archive() -> Path:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        fd, temp_zip = tempfile.mkstemp(prefix="rebecca_backup_", suffix=".zip")
        os.close(fd)
        final_path = Path(temp_zip)
